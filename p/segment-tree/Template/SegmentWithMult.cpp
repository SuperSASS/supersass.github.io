...
struct typeSegment
{
	...
	void modify_M(int index, const int &m_L, const int &m_R, const int &m_val)
	{
		...
		else
		{
			if (NOW_Node.lazy_P) //如果存在加的lazy
			{
				if (NOW_Node.lazy_M != 1) NOW_Node.pushdown_M(*this);
				NOW_Node.pushdown_P(*this);
			}
			/*
			if (m_L <= NOW_Node.L && m_R <= NOW_Node.R)									   //左边超出
				NOW_Node.val = query(1, m_R + 1, NOW_Node.R) * (m_val - 1); //！！！这里存在减法，可能MOD出错
			...
			*/
			...
			NOW_Node.val = ((query(NOW_LSon, NOW_Node.L, NOW_Node.mid) % MOD) + (query(NOW_RSon, NOW_Node.mid + 1, NOW_Node.R) % MOD)) % MOD;
		}
	}
	...
	long long query(int index, const int &q_L, const int &q_R)
	{
		...
		long long tmpAns = 0;
		if (q_L <= NOW_Node.mid)
			tmpAns = (tmpAns + query(NOW_LSon, q_L, q_R)) % MOD;
		//如果要查询区间左端点，比当前区间的中点还大，证明左儿子区间完全不在
		if (q_R > NOW_Node.mid)
			tmpAns = (tmpAns + query(NOW_RSon, q_L, q_R)) % MOD;
		return tmpAns;
	}
